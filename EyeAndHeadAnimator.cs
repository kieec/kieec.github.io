	...
	public class EyeAndHeadAnimator : MonoBehaviour
	{
		#region fields
			...
			#region head
				...
				[HideInInspector] public float headWeight = 0;
				[HideInInspector] public Transform headBoneNonMecanimXform;
				Quaternion headBoneNonMecanimFromRootQ;

		//Custom:J
		public bool setHeadWeightZero = false;
		OVRLipSyncContextMorphTarget oVRLipSyncContextMorphTargetTemp;
				// Head jitter
			...
			Animator animator;
        //Custom:J
        ControlFacialAnim ctrlFacialAnim;
		...
		void Start()
		{
            ...
            //Custom: J
            ctrlFacialAnim = this.GetComponent<ControlFacialAnim>();
            lookTargetController = this.GetComponent<LookTargetController>();
			oVRLipSyncContextMorphTargetTemp = this.GetComponent<OVRLipSyncContextMorphTarget>();
            SetPointsOfInterestRandomly();
        }


        void SetPointsOfInterestRandomly() {
            if (lookTargetController.pointsOfInterest == null || lookTargetController.pointsOfInterest.Length == 0) {
                lookTargetController.pointsOfInterest = new Transform[9];

                Vector3 leftDirection = (leftEyeAnchor.parent.rotation * leftEyeAnchor.localRotation * leftAnchorFromEyeRootQ) * Vector3.forward;
                Vector3 pofRoot = this.transform.Find("CC_Base_BoneRoot").transform.position;//leftEyeAnchor.position + rightEyeAnchor.position;
                pofRoot += (Vector3.up * 1.3f);

                for (int i = 0; i < 9; i++)
                {
                    lookTargetController.pointsOfInterest[i] = new GameObject().transform;
					lookTargetController.pointsOfInterest[i].parent = oVRLipSyncContextMorphTargetTemp.jawRoot.transform.parent;
					lookTargetController.pointsOfInterest[i].name = "PointsOfInterest_" + i;                    
                }
                float offset = Random.Range(0.05f, 0.15f);

                lookTargetController.pointsOfInterest[0].position = pofRoot + leftDirection * 1.0f;
                lookTargetController.pointsOfInterest[1].position = lookTargetController.pointsOfInterest[0].position - Vector3.right * offset;
                lookTargetController.pointsOfInterest[2].position = lookTargetController.pointsOfInterest[0].position + Vector3.right * offset;
                lookTargetController.pointsOfInterest[3].position = lookTargetController.pointsOfInterest[0].position + Vector3.up * offset;
                lookTargetController.pointsOfInterest[4].position = lookTargetController.pointsOfInterest[3].position - Vector3.right * offset;
                lookTargetController.pointsOfInterest[5].position = lookTargetController.pointsOfInterest[3].position + Vector3.right * offset;
                lookTargetController.pointsOfInterest[6].position = lookTargetController.pointsOfInterest[0].position - Vector3.up * offset;
                lookTargetController.pointsOfInterest[7].position = lookTargetController.pointsOfInterest[6].position - Vector3.right * offset;
                lookTargetController.pointsOfInterest[8].position = lookTargetController.pointsOfInterest[6].position + Vector3.right * offset;


            }
        }

        public void UpdatePointsOfInterestTransform() {
		...
		void Update()
		{
			if (!setHeadWeightZero) {
				headWeight = Mathf.Clamp01 (Mathf.PerlinNoise (Time.time, 0.0f));
			} else {
				headWeight = 0.0f;
			}
		...
		void UpdateBlinking()
		{
			...
			if ( blinkState != BlinkState.Idle )
			{
				...
				if ( blinkStateTime >= blinkDuration )
				{
					...
					{
                        Debug.Log("Closing");
                        ctrlFacialAnim.AnimBrow();
                        //ctrlFacialAnim.AnimNose();
                        //ctrlFacialAnim.AnimCheek();
                        ctrlFacialAnim.AnimMouth();
